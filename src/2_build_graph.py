...
# Ensure this matches the name of the file where you saved the TextGCNGraph class
from graph_builder import TextGCNGraph 

def main():
    print("=== STEP 2: Offline Graph Construction (Pure Semantic Mode) ===")
    ...
    # Calculate Node-Word and Word-Word Edges
    tfidf_matrix = graph_builder.build_tfidf_edges()
    pmi_edges = graph_builder.build_pmi_edges(window_size=20)
    
    # Calculate Semantic Edges (Cosine Similarity Only)
    # We use a strict threshold (0.85) to ensure we only connect tweets with highly similar meanings
    semantic_edges = graph_builder.build_semantic_doc_edges(doc_features)
    
    # 4. Assemble the Master Adjacency Matrix (Jaccard Removed)
    A_matrix = graph_builder.build_adjacency_matrix(pmi_edges, semantic_edges=semantic_edges)
    ...
    print(f"\n[SUCCESS] Upgraded Semantic Adjacency Matrix saved to: {save_path}")
